...
# ╔═╡ b6efb34e-e3b3-4123-ab0a-cde08e313d70
md"""
### Kalash Pai (krp57) and Dharani Prasad Vadlamudi (dv235)

Kalash Pai - Formulating the u array using given formula.

Dharani Prasad Vadlamudi - Writing the flux bounds array and the code that gave results

We figured out Case 2 where we include metabolite data in the bounds has no effect on final result.
"""
...
# ╔═╡ 8b318a36-d81a-4798-97c6-cb95b747a581
# FORMULATING THE U ARRAY
begin
	# Replacement Strategy
	replacement_flag = 0 # if 1 use geomean, else use 0
	...
	if (replacement_flag == 1)
		# compute geomean of Sj/kj values
		gmean_sk = geomean(metabolite_table[!,:"[Met]/Km"])
		sk_array = (gmean_sk)*u2 # units: μmol/gDW
	end

	# building the table
	reactions_in_our_model_array = ["v1","v2","v3","v4","Fv5","Rv5"]
	substrate_in_our_model_array = [["ATP","citrulline","aspartate"],["M_N-(L-Arginio)succinate_c"],["arginine","M_Oxygen_c","nadph","M_H_c"],["M_Nitric_oxide_c","citrulline","nadp","M_H2O_c"]]
	ec_number_array_modified = vcat(ec_number_array,"1.14.13.39")

	for (i,ec_number) in enumerate(ec_number_array_modified)
		df_reaction = filter([:"EC Number",:"Metabolite"]=>(x,y)->(x == ec_number && y == substrate_in_our_model_array),metabolite_table)

		# is the df empty?
		if (isempty(df_reaction) == false)
			sk_array[i] = prod(df_reaction[!,:"[Met]/Km"])
			denominator[i] = prod(ones(i) + df_reaction[!,:"[Met]/Km"])
		else

			if (replacement_flag == 1)
				sk_array[i] = gmean_sk
			end
			denominator[i] = 1 + sk_array[i]
		end
		u2[i] = sk_array[i]/(denominator[i]-1)
	end
end

# ╔═╡ 23a9047a-77c5-4235-a090-69006b1d7ced
begin
	...
	flux_bounds_array2[1,2] = 203.0*E2[1]
	flux_bounds_array2[2,2] = 34.5*E2[2]
	flux_bounds_array2[3,2] = 249.0*E2[3]
	flux_bounds_array2[4,2] = 88.1*E2[4]
	flux_bounds_array2[5,2] = 13.7*E2[5]
	flux_bounds_array2[6,2] = 13.7*E2[6]
...
# ╔═╡ 5f866770-9937-4118-805e-cf51e1f18c5b
# check:
with_terminal() do
...
# ╔═╡ c5f6de96-61f0-43ca-846f-625b00016250
let
